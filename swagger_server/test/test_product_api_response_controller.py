...
from swagger_server.models.product_locations import ProductLocations
from swagger_server.models.product_categories import ProductCategories
from swagger_server.models.product_lists import ProductLists
...
class TestProductApiResponseController(BaseTestCase):
    ...
    def test_post_product(self):
        ...
        """
        lc=ProductLocations("2","Product Location")
        cg=ProductCategories("3","Product Category")
        cy=Companies("5","Company name","Company Image","Company Address")
        cr=Contractors("6","Contractor name","Contractor Email","0123456789","Contractor Address")
        pl=ProductLists("4","1","abc","excellent",cy,cr)
        body = ProductApiResponse("1",lc,cg,pl)
        response = self.client.open(
            '/api//products',
            method='POST',
            data=json.dumps(body),
            content_type='application/json')
        self.assert200(response,
                       'Response body is : body' + response.data.decode('utf-8'))
