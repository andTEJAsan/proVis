...
def get_contractor(id):  # noqa: E501
    ...
    """
    contractor_id = id
    username = db.get_username_fromcontractor(id)
    email = db.get_email_fromcontractor(id)
    phone_number = db.get_phone_fromcontractor(id)
    address=db.get_address_fromcontractor(id)
    company=db.get_codmpanies_fromcontractor(id)
    product=db.get_apis_fromcontractor(id)
    order=db.get_orders_fromcontractor(id)
    return ContractorApiResponse(contractor_id=contractor_id,username=username,emailid=email,phone_number=phone_number,address=address,company=company,product=product,order=order)
