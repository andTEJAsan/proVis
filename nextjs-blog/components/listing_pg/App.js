...
function App() {
  ...
  let new_card_array = products.map((obj) => (
    <Card
      clicker={() => router.push("/product_pg", { query: query_obj })}
      obj={gen_obj(obj)}
      key={obj.p_uid}
    />
  ));
...
//   })
  ...
  useEffect(() => {
    fetch(
      `${apiUrl}/api/products?location=${newState.location}&category=${newState.category}`,
      {
        method: "GET",
        headers: {
      ...
      .then((response) =>
        response.json().then((data) => {
          ...
          let query_obj = {
            company_img_url: data.company_img_url,
